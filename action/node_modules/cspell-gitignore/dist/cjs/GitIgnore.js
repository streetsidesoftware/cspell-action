...
const GitIgnoreFile_js_1 = require("./GitIgnoreFile.js");
...
class GitIgnore {
    ...
    async _findGitIgnoreHierarchy(directory) {
        ...
        const parentHierarchy = parent !== directory && (0, helpers_js_1.contains)(root, parent) ? await this.findGitIgnoreHierarchy(parent) : undefined;
        const git = await (0, GitIgnoreFile_js_1.loadGitIgnore)(directory);
        if (!git) {
            return parentHierarchy || new GitIgnoreFile_js_1.GitIgnoreHierarchy([]);
        }
        const chain = parentHierarchy?.gitIgnoreChain.concat([git]) ?? [git];
        return new GitIgnoreFile_js_1.GitIgnoreHierarchy(chain);
    }
    determineRoot(directory) {
        const roots = this.roots;
        for (let i = roots.length - 1; i >= 0; --i) {
            const r = roots[i];
            if ((0, helpers_js_1.contains)(r, directory))
                return r;
        }
