...
function resolveFilename(filename, cwd) {
    cwd = cwd || process.cwd();
    if (filename === constants_1.STDIN)
        return constants_1.STDINProtocol;
    if (filename.startsWith(constants_1.FileProtocol)) {
        const url = new URL(filename.slice(constants_1.FileProtocol.length), (0, url_1.pathToFileURL)(cwd + path.sep));
        return (0, url_1.fileURLToPath)(url);
    }
    const scheme = filename.startsWith(constants_1.STDINProtocol) ? constants_1.STDINProtocol : '';
...
function readFile(filename, encoding = constants_1.UTF8) {
...
async function findFiles(globPatterns, options) {
    const stdin = [];
    const globPats = globPatterns.filter((filename) => filename !== constants_1.STDIN && !filename.startsWith(constants_1.STDINProtocol) && !filename.startsWith(constants_1.FileProtocol)
        ? true
