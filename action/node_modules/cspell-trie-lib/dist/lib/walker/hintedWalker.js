...
function* hintedWalkerNext(root, ignoreCase, hint, compoundingMethod, emitWordSeparator = '') {
    ...
    const rawRoots = [root, ignoreCase ? root.c.get(noCaseCharacter) : undefined].filter(isDefined_1.isDefined);
    ...
    const compoundRoots = rawRoots.map((r) => r.c?.get(compoundCharacter)).filter(isDefined_1.isDefined);
