...
function suggest(root, word, numSuggestions = defaultMaxNumberSuggestions, compoundMethod = walker_1.CompoundWordsMethod.NONE, numChanges = maxNumChanges, ignoreCase) {
...
function* genCompoundableSuggestions(root, word, compoundMethod) {
    ...
    stack[0] = { a, b };
    const hint = word;
    const iWalk = (0, walker_1.hintedWalker)(root, compoundMethod, hint);
    let goDeeper = true;
    for (let r = iWalk.next({ goDeeper }); !r.done; r = iWalk.next({ goDeeper })) {
        ...
        if (node.f && (0, util_1.isWordTerminationNode)(node) && cost <= costLimit) {
