...
class Trie {
    constructor(root, count) {
        this.root = root;
        this.count = count;
        this._options = (0, util_1.mergeOptionalWithDefaults)(root);
    ...
    size() {
        var _a;
        this.count = (_a = this.count) !== null && _a !== void 0 ? _a : (0, util_1.countWords)(this.root);
        return this.count;
    }
    isSizeKnown() {
    ...
    completeWord(text) {
        ...
        const subNodes = (0, util_1.iteratorTrieWords)(n || {})
            .filter((w) => w[w.length - 1] !== compoundChar)
            .map((suffix) => text + suffix);
        return (0, gensequence_1.genSequence)(n && (0, util_1.isWordTerminationNode)(n) ? [text] : []).concat(subNodes);
    }
    /**
     ...
     */
    suggest(text, maxNumSuggestions, compoundMethod, numChanges, ignoreCase) {
     ...
     */
    suggestWithCost(text, maxNumSuggestions, compoundMethod, numChanges, ignoreCase) {
        return (0, suggest_1.suggest)(this.getSuggestRoot(!ignoreCase), text, maxNumSuggestions, compoundMethod, numChanges, ignoreCase).filter((sug) => !this.isForbiddenWord(sug.word));
    }
    /**
    ...
    genSuggestions(collector, compoundMethod) {
        ...
        const suggestions = (0, suggest_1.genSuggestions)(this.getSuggestRoot(!collector.ignoreCase), collector.word, compoundMethod);
     ...
     */
    words() {
        return (0, util_1.iteratorTrieWords)(this.root);
    }
    /**
    ...
    insert(word) {
        (0, util_1.insert)(word, this.root);
        return this;
    }
    getSuggestRoot(caseSensitive) {
        ...
        if (!caseSensitive && (csRoot === null || csRoot === void 0 ? void 0 : csRoot.c)) {
            roots.push({
    ...
    static create(words, options) {
        const root = (0, util_1.createTriFromList)(words, options);
        (0, util_1.orderTrie)(root);
