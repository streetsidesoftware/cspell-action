...
function listGlobalImports() {
    const globalSettings = GlobalSettings_1.getRawGlobalSettings();
...
function addPathsToGlobalImports(paths) {
    ...
    const rawGlobalSettings = GlobalSettings_1.getRawGlobalSettings();
    ...
    const error = GlobalSettings_1.writeRawGlobalSettings(globalSettings);
    return {
        success: !error,
...
function removePathsFromGlobalImports(paths) {
    ...
    const error = toRemove.size > 0 ? GlobalSettings_1.writeRawGlobalSettings(updatedSettings) : undefined;
    return {
        success: true,
        removed: [...toRemove],
...
function resolveSettings(filename) {
    var _a;
    const settings = CSpellSettingsServer_1.readRawSettings(filename);
