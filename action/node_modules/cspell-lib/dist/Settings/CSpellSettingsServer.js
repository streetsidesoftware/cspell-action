...
const cspellCosmiconfig = {
    ...
    loaders: {
        ...
        '.jsonc': (_filename, content) => json.parse(content),
    },
};
...
function readConfig(fileRef) {
    ...
    try {
        ...
        fileRef.error = new ImportError_1.ImportError(`Failed to read config file: "${filename}"`, err);
    }
 ...
 */
function normalizeSettings(rawSettings, pathToSettingsFile) {
...
async function normalizeSearchForConfigResult(searchPath, searchResult) {
    var _a;
    let result;
    let error;
    try {
...
function searchForConfig(searchFrom) {
    return normalizeSearchForConfigResult(searchFrom || process.cwd(), cspellConfigExplorer.search(searchFrom)).then((r) => (r.filepath ? r.config : undefined));
}
...
function normalizeDictionaryDefs(settings, pathToSettingsFile) {
    var _a;
    const dictionaryDefinitions = DictionarySettings_1.normalizePathForDictDefs(settings.dictionaryDefinitions, pathToSettingsFile);
    const languageSettings = (_a = settings.languageSettings) === null || _a === void 0 ? void 0 : _a.map((langSetting) => util.clean({
        ...langSetting,
        dictionaryDefinitions: DictionarySettings_1.normalizePathForDictDefs(langSetting.dictionaryDefinitions, pathToSettingsFile),
    }));
    return util.clean({
