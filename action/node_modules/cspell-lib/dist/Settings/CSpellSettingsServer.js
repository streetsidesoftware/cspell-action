...
function loadPnP(pnpSettings, searchFrom) {
    ...
    const loader = pnpLoader_1.pnpLoader(pnpSettings.pnpFiles);
...
function validateRawConfigVersion(config, fileRef) {
    ...
    if (version === undefined || supportedCSpellConfigVersions.includes(version))
        return;
    if (!/^\d+(\.\d+)*$/.test(version)) {
        logger_1.logError(validationMessage(`Unsupported config file version: "${version}"`, fileRef));
        return;
    }
    ...
    logger_1.logWarning(validationMessage(msg, fileRef));
}
