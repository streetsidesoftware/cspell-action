...
const AutoResolveLRUCache_1 = require("../../../util/AutoResolveLRUCache");
...
const PnPSettings_1 = require("./PnPSettings");
...
class ConfigLoader {
    ...
    constructor(cspellIO) {
        ...
        this.searchConfigLRU = new AutoResolveLRUCache_1.AutoResolveLRUCache(CACHE_SIZE_SEARCH_CONFIG, (a, b) => a.searchFrom === b.searchFrom && a.pnpSettings === b.pnpSettings);
    }
    readSettings(filename, relativeToOrDefault, defaultValue) {
        ...
        return this.importSettings(ref, defaultValue, defaultValue || PnPSettings_1.defaultPnPSettings);
    }
    async readSettingsAsync(filename, relativeTo, pnpSettings) {
        const ref = resolveFilename(filename, relativeTo || process.cwd());
        return this.importSettings(ref, undefined, pnpSettings || PnPSettings_1.defaultPnPSettings);
    }
    /**
     *
     ...
     */
    searchForConfig(searchFrom, pnpSettings = PnPSettings_1.defaultPnPSettings) {
        pnpSettings = (0, PnPSettings_1.normalizePnPSettings)(pnpSettings);
    ...
    normalizeSettings(rawSettings, pathToSettingsFile, pnpSettings) {
        ...
        const pnpSettingsToUse = (0, PnPSettings_1.normalizePnPSettings)({ usePnP, pnpFiles });
        const pathToSettingsDir = path.dirname(pathToSettingsFile);
        loadPnPSync(pnpSettingsToUse, (0, Uri_1.toUri)(pathToSettingsDir));
 ...
 * @returns the resulting settings
 */
function searchForConfig(searchFrom, pnpSettings = PnPSettings_1.defaultPnPSettings) {
 ...
 */
function searchForConfigSync(searchFrom, pnpSettings = PnPSettings_1.defaultPnPSettings) {
    pnpSettings = (0, PnPSettings_1.normalizePnPSettings)(pnpSettings);
    let searchResult;
    try {
 ...
 */
async function loadConfig(file, pnpSettings = PnPSettings_1.defaultPnPSettings) {
 ...
 * @deprecated
 */
function loadConfigSync(filename, pnpSettings = PnPSettings_1.defaultPnPSettings) {
    const pnp = (0, PnPSettings_1.normalizePnPSettings)(pnpSettings);
