...
function* suggestIteration(i, word, minScore = defaultMinScore) {
    let goDeeper = true;
    const fA = (0, helpers_js_1.wordToFeatures)(wPrefix + word + wSuffix);
    for (let r = i.next(goDeeper); !r.done; r = i.next(goDeeper)) {
        const { text, node } = r.value;
        const fB = (0, helpers_js_1.wordToFeatures)(wPrefix + text);
        ...
        if (goDeeper && node.f) {
            const fB = (0, helpers_js_1.wordToFeatures)(wPrefix + text + wSuffix);
            const rawScore = fA.intersectionScore(fB);
