...
class SpellingDictionaryCollection {
    constructor(dictionaries, name, wordsToFlag) {
        ...
        this._isForbiddenInDict = Memorizer_1.memorizer((word) => isWordForbiddenInAnyDictionary(this.dictionaries, word), SpellingDictionaryFromTrie_1.SpellingDictionaryFromTrie.cachedWordsLimit);
    ...
    _suggest(word, suggestOptions) {
        ...
        const collector = SpellingDictionaryMethods_1.suggestionCollector(word, {
            numSuggestions,
            filter,
...
function isWordForbiddenInAnyDictionary(dicts, word) {
    return gensequence_1.genSequence(dicts).first((dict) => dict.isForbidden(word));
}
