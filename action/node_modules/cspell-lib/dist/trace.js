...
async function traceWords(words, settings, options) {
    ...
    async function finalize(config) {
        ...
        const dictBase = await (0, SpellingDictionary_1.getDictionary)(settings);
        const dicts = await (0, SpellingDictionary_1.getDictionary)(dictSettings);
    ...
    const opts = { ignoreCase, useCompounds: config.allowCompoundWords };
    const r = await Promise.all((0, gensequence_1.genSequence)(words)
    ...
    const s = (0, gensequence_1.genSequence)(r)
        .concatMap((p) => {
