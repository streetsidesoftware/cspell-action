...
async function push(context, github) {
    ...
    const files = commits && await github_2.fetchFilesForCommits(github, context.repo, commits);
    if (files) {
        await checkSpelling(files);
    }
}
...
async function checkSpelling(files) {
    const result = await spell_1.lint(files, { root: process.cwd() }, core);
...
async function action() {
    ...
    const github = github_1.getOctokit(getGithubToken());
